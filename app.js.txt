...
function renderScorecard(innings) {
  let batHTML = `<tr><th>Batter</th><th>R</th><th>B</th><th>SR</th></tr>`;
  ...
  let bowlHTML = `<tr><th>Bowler</th><th>O</th><th>R</th><th>W</th><th>ER</th></tr>`;
  for (let p in innings.bowlingCard) {
    let b = innings.bowlingCard[p];
    let overs = (b.balls / 6).toFixed(1);
    let er = b.overs ? (b.runs / b.overs).toFixed(2) : "0.00";
    bowlHTML += `<tr>
      <td>${p}</td>
      <td>${overs}</td>
      <td>${b.runs}</td>
